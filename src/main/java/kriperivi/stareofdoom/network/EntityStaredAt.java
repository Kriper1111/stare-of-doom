...
public class EntityStaredAt implements IMessage, IMessageHandler<EntityStaredAt, IMessage> {
    ...
    public IMessage onMessage(EntityStaredAt message, MessageContext ctx) {
        ...
        if (!CONFIG.getServerConfig().doesMobApply(target))
            return null;
        ...
        int threshold = CONFIG.getServerConfig().getStareThreshold();
        if (stareDuration < threshold)
            return null;
        else
